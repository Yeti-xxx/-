        ...
        <div class="search-item search-item-transition" style="top: 45px;">
          ...
          <div class="all-items">
            ...
            <el-tag v-for='item in secondCategorys' :key='item.id'
              ...
              effect="plain" type="info" @click='buildingCondition("scategory", item.id)'> {{ item.categoryName }}
            </el-tag>
          </div>
        </div>
        <div class="search-item" style="top: 90px;">
          ...
          <div class="all-items">
            ...
            <el-tag v-for='item in courseLevel' :key='item.id'
              :class="currentType['clevel'].id == item.code ? 'category-poniter' : 'category-poniter-item'" effect="plain"
              type="info" @click='buildingCondition("clevel", item.code)'> {{ item.text }}
            </el-tag>
          </div>
        </div>
      </section>
        ...
        <div class="newCourseContent">
          <ul class="courseUl">
            <li class='courseItem' v-for='item in courseList' :key='item.id'>
              <RouterLink :to="{ path: '/course-info/' + item.id }">
