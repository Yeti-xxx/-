    ...
    <div class="container">
      <div class="main">
        ...
        <div v-if="cartList.length > 0">
          <ul class="haveorder" v-for="item in cartList" :key="item.id">
            <li class="order-item">
              <el-checkbox v-model="item.check" @change="cartStore.itemChecked(item.id)"></el-checkbox>
            </li>
...
import { getShopCartList,deletShopCart } from '../utils/api/cart';
// pinia
import { useCartStore } from '../store/cart'
// pinia的storeToRefs 进行数据解构
import { storeToRefs } from 'pinia';
// 临时token
import { createToken } from '../utils/api/createToken';
import { useRouter } from 'vue-router';
const cartStore = useCartStore()
const { cartList, isChecked, total } = storeToRefs(cartStore)
